...
function sdm_generate_fancy4_popular_downloads_display_output( $get_posts, $args ) {

	wp_enqueue_style( 'sdm_generate_fancy4_popular_downloads_display_styles', WP_SIMPLE_DL_MONITOR_URL . '/includes/templates/fancy4/sdm-fancy-4-styles.css' , array(), WP_SIMPLE_DL_MONITOR_VERSION );
    $output = '<div class="sdm_fancy4_flex_container">';

    foreach ( $get_posts as $item ) {
	$opts		 = $args;
	$opts[ 'id' ]	 = $item->ID;
	$output		 .= sdm_generate_fancy4_display_output( $opts );
    }
    $output .= '</div><div class="sdm_clear_float"></div>';
    return $output;
}

function sdm_generate_fancy4_latest_downloads_display_output( $get_posts, $args ) {

	wp_enqueue_style( 'sdm_generate_fancy4_latest_downloads_display_styles', WP_SIMPLE_DL_MONITOR_URL . '/includes/templates/fancy4/sdm-fancy-4-styles.css' , array(), WP_SIMPLE_DL_MONITOR_VERSION );
    $output = '<div class="sdm_fancy4_flex_container">';

    foreach ( $get_posts as $item ) {
	$output .= sdm_generate_fancy4_display_output(
...
function sdm_generate_fancy4_category_display_output( $get_posts, $args ) {

	wp_enqueue_style( 'sdm_generate_fancy4_category_display_styles', WP_SIMPLE_DL_MONITOR_URL . '/includes/templates/fancy4/sdm-fancy-4-styles.css' , array(), WP_SIMPLE_DL_MONITOR_VERSION );
    $output = '<div class="sdm_fancy4_flex_container">';
...
function sdm_generate_fancy4_display_output( $args ) {
    ...
    $download_button_code	 = '<a href="' . $download_url . '" target="' . $window_target . '">' . $button_text . '</a>';
    //Get item file size
    ...
    $output	 .= '<div class="sdm_download_item sdm_fancy4_flexline_container ' . esc_attr($css_class) . '">';
    //$output	 .= '<div class="sdm_download_item_top">';
    $output	 .= '<div class="sdm_download_thumbnail sdm_fancy4_flex_item">' . $isset_download_thumbnail . '</div>';
    $output	 .= '<div class="sdm_fancy4_download_title sdm_fancy4_flex_item">' . esc_html($item_title) . '</div>';	
    $output  .= '<div class="sdm_download_button sdm_fancy4_flex_item">' . $download_button_code . '</div>';
